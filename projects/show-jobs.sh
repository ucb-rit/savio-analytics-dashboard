...
while IFS='|' read -a LINE; do
  ...
  echo "jobs,job_id=${LINE[0]},account=${ACCOUNT},type=${TYPE[0]},department=$(printf '%q' "${DEPARTMENT[2]}"),cpus=${LINE[4]},partition=${LINE[5]},state=${STATE[0]} req_nodes=${LINE[8]},alloc_nodes=${LINE[9]},raw_time=${LINE[10]},cpu_time=${LINE[11]} $END_TIMESTAMP"
done
